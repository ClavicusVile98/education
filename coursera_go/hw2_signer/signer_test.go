...
/********************************************************************************
*	This is a test to verify that this is really a pipeline of functions	*
*										*
*	Incorrect behaviour: accumulating the results of the execution of one	*
*	function and then sending it to the next one.				*
********************************************************************************/
func TestPipeline(t *testing.T) {
	...
	freeFlowJobs := []job{
		job(func(in, out chan interface{}) {
			...
			// What is the point?
			// The accumulation of values until the whole function runs out
			// will not passed it further
			// Here is some check for the counter increasing in the next
			// function. This means that it is received the values before
			// the current function runs out
			if currRecieved == 0 {
				ok = false
			}
		}),
